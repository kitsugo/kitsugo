## Greetings

I'm Kitsugo, I like coding, computer science and foxes. 🦊\
All my repositories have moved to my [Git page](https://git.kitsugo.com/), but I keep <ins>empty</ins> commit mirrors here on GitHub.

## About me

- I teach computer science, and I'm especially passionate about computer systems. 🖥
- Sometimes I program, focusing mostly on smaller projects and tools. 🛠
- I'm quite invested in free software, online privacy and digital rights in general. 🐧

Feel free to visit [my website](https://kitsugo.com/) for more information about me and what I do.

## Tech stack

- ~~Webslop~~ Webdev (HTML, CSS, Typescript, SolidJS)
- Java
- Python
- Lua
- Linux and Homelab (Bash, Ansible)

[![Icons of technologies I use](https://skillicons.dev/icons?i=ts,html,css,solidjs,astro,java,py,bash,lua,ansible,linux)](https://skillicons.dev)

## Contact

You can reach out to me via my personal e-mail: `kitsu[AT]kitsugo.com`.

[![ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/kitsugo)

<br/>

<img width="600" height="397" alt="image" src="https://github.com/user-attachments/assets/05c9f305-cbf9-49c5-9bfb-9295f841f4f5" />
